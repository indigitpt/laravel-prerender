...
namespace Indigit\LaravelPrerender\Http\Middleware;
...
class PrerenderMiddleware
{
    ...
    public function __construct(Application $app, Guzzle $client)
    {
        ...
        $this->prerenderUsername = $config['username'] ?? null;
        $this->prerenderPassword = $config['password'] ?? null;
    }
    ...
    private function getPrerenderedPageResponse($request)
    {
        ...
        try {
            ...
            // Request URI
            $requestUri = $this->prerenderUri . '/' . urlencode($protocol . '://' . $host . '/' . $path);
            ...
            // Include Auth Params
            if(!empty($this->prerenderUsername) && !empty($this->prerenderPassword)){
                $guzzleOptions['auth'] = [
                    $this->prerenderUsername,
                    $this->prerenderPassword
                ];
            }
            // Call pre-render
            return $this->client->get($requestUri,$guzzleOptions);
            ...
            if ($this->app['config']->get('prerender.debug') && $this->app['config']->get('app.debug')) {
                throw $exception;
            }
            return null;
        }
